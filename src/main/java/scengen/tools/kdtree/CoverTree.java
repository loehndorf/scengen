 ...
 * TODO: There is still an unknown error in the removeNodes(int kCenters).
 *
 */
...
class CoverTree<E>  {
	 ...
	 */
	public CoverTree (double[] weights) {
	 ...
	 */
	public CoverTree (double base, int maxMinLevel, double[] weights) {
		this.base = base;
	 ...
	 * @return
	 */
	public List<Node<E>> getFullCover(int level) {
		List<Node<E>> coverset = new LinkedList<Node<E>>();
		coverset.add(rootNode);
		int k = maxLevel;
		while(k-- > level){
